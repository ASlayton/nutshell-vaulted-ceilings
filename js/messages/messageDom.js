    ...
    .then((allUsers) => {
      ...
      messageArray.forEach((message) => {
        if (message.uid === firebase.auth().currentUser.uid) {
          messagesString += `<div class="row text-right">`;
          messagesString += `<div class="user-speech-bubble message-card col-md-12" data-firebase-id="${message.id}">`;
          messagesString += `<p class="message-text">${message.message}</p>`;
          messagesString += `<p class="message-username">${message.userName}</p>`;
